		...
		try {
			...
			if(rs.next()) {
				...
				else if(typ.equals("student"))
				{
					...
					Vector v=jf.getValue("select semester,course from studentpersonal where userid='"+userid.trim()+"'", 2);
				...
				{
					sess.setAttribute("collegeid", rs.getString("userid"));
			...
			System.out.println("errlogin="+ex.getMessage());
			//ex.printStackTrace();
		}
