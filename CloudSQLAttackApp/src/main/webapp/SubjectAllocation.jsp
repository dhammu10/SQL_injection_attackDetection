...
<table class="tblform">
	<!-- <tr>
		...
		<tr>
		<td>Staff Name</td>
		<td><select name="staffName" required class="form-control" onchange="makeGetRequest(this.value)">
		<option value=""><---Select---></option> 
		 <%
		 List<Staff> lststaff=(List<Staff>)request.getAttribute("lstStaff");
		 for(int i=0;i<lststaff.size();i++)
		 {
			 Staff st=(Staff)lststaff.get(i);
			 %><option value="<%=st.getUserid() %>"><%=st.getUsernm() %></option><%
		 }
		 %>	</select></td></tr>
	<tr>
		<td>Branch Name</td>
