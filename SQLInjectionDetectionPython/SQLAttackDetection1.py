...
examid,userid,id,input = message.split('|')
print(message) 
...
UPLOAD_DIR_Model=os.getcwd()
basedir = UPLOAD_DIR_Model+"/trainingdata/"
...
#X = dataframe.to_numpy(['token_length', 'entropy','sqli_g_means','plain_g_means'])
...
#check_data = "%' or '0'='0"
check_data="'999' UNION SELECT Username, Password FROM Users"
res = Check_is_sql(input)
...
print("<html>")
print("<head>")
print("<meta http-equiv='refresh' content='0;url=http://localhost:8080/fromPythonStud1?sts="+param+"&result="+result+"&examId="+examid+"&userid="+userid+"&ansid="+id+"'/>")
